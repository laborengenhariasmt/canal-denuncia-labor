  ...
  .addEventListener("submit", async function (e) {
    ...
    try {
      const resposta = await fetch(
        `/api/consultar-protocolo?protocolo=${encodeURIComponent(protocolo)}`
      );

      const dados = await resposta.json();

      if (!resposta.ok) {
        throw new Error(dados.erro || "Não foi possível consultar o protocolo.");
      }
      ...
      resultado.innerHTML = `
        <div class="aviso">
          ...
          <p>
            Para preservar a confidencialidade, esta consulta mostra
            somente o andamento do relato.
          </p>
        </div>
      `;
    } catch (erro) {
